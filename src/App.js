...
function Page({ text, tag, images, textScaleFactor, onReflow, left = false }) {
  ...
  const boxProps = {
    ...
    minWidth: 3,
    minHeight: 3,
    aspectRatio: 1, // set the aspect ratio of the box
  }
  return (
    <Box dir="column" align={left ? 'flex-start' : 'flex-end'} justify="flex-start" width="100%" height="auto" minHeight="100%">
      ...
      <Box dir="row" width="100%" height="auto" justify={left ? 'flex-end' : 'flex-start'} margin={0} grow={1} wrap="wrap">
        {textures.map((texture, index) => (
          <Box {...boxProps} key={index} className="box">
            {(width, height) => {
              // calculate the width and height based on the aspect ratio
              if (width / height > boxProps.aspectRatio) {
                height = width / boxProps.aspectRatio
              } else {
                width = height * boxProps.aspectRatio
              }
              return (
                <mesh>
      ...
      <Box marginLeft={1.5} marginRight={1.5} marginTop={2}>
        <Box marginLeft={left ? 2 : 1} marginRight={left ? 1 : 2} marginBottom={1}>
          <Text
            bold
            position-z={0.5}
